...
## 2025-05-28 - Toast Notifications vs Alerts
**Learning:** Native `alert()` dialogs block the main thread and disrupt the user flow, creating a jarring experience.
**Action:** Replace blocking alerts with non-blocking toast notifications. Use `role="alert"` for errors and `role="status"` for success messages. Ensure the container passes through pointer events (`pointer-events-none`) so it doesn't block underlying UI, while making individual toasts interactive (`pointer-events-auto`).
