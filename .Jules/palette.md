...
## 2025-05-27 - Toast Notification Pattern
**Learning:** Replacing native alerts with custom toasts improves UX but introduces XSS risks if not handled carefully.
**Action:** When creating dynamic UI components like toasts, always separate the structural HTML (safe to use `innerHTML`) from the dynamic content. Use `textContent` to insert the message text safely. Ensure proper accessibility with `role="alert"` (for errors) or `role="status"` (for updates) and `aria-live` attributes.
